...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    @commands.command(name="mute", description="Mutes a member for a specific amount of time.")
    @commands.has_permissions(manage_messages=True)
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    async def modban(self, ctx, member: nextcord.Member, *, reason=None):
        ...
        if view.value is None:
            ...
            guild = ctx.guild
            log_channel = self.bot.get_channel(920114324029198336)
            banMsg = random.choice(ban_msg)
            banEmbed = nextcord.Embed(
                title="Ban Success", description=f"{member.mention} {banMsg}"
            )
            ...
            #await member.send(f"You got banned in **{guild}** | Reason: **{reason}**")
            logEmbed = nextcord.Embed(title=f"**{member.name}** has ben banned in **{guild}**")
            ...
            await ctx.send(embed=banEmbed)
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    @commands.command(name="idban", description="Bans the member from your server with their id.")
    @commands.has_permissions(ban_members=True)
    async def idban(self, ctx, id: int, *, reason=None):
        if reason is None:
            reason = f"{ctx.author.name} banned {member.name}"
        else:
            reason = (f"{ctx.author.name} banned {member.name} for the reason of {reason}")
